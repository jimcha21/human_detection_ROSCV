...
int main(int argc, char** argv)  //Mainnnnnnnnnnnnnnnnnnnnnnn
{

	ros::init(argc, argv, "stereo_match");
	ros::NodeHandle n;
...
{
 // Load images
	left_src2 = imread(p.left);
    right_src2 = imread(p.right);
    if (left_src2.empty()) throw runtime_error("can't open file \"" + p.left + "\"");
    if (right_src2.empty()) throw runtime_error("can't open file \"" + p.right + "\"");

	Size size(400,400);
	resize(left_src2,left_src,size);//resize image
	resize(right_src2,right_src,size);//resize image
	/*ros::Subscriber sub = n.subscribe("/guidance/left_image", 1, chatterCallback2);
	return;
	*/
